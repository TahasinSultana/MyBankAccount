...
if(isset($_POST['submit'])){
	...
	if(empty($Account_number)||empty($Amount_to_deposit) || empty($Account_name) || empty($Depositor_name)){
		header('Location:\BankProject/EasyBank/HomePage.php? please fill the space ');
		exit();
	}
		...
		if(mysqli_num_rows($run_query) > 0 ){
			while ($row_data = mysqli_fetch_assoc($run_query)) {
				...
				$deposit_account = "INSERT INTO deposit(accountnumber,accountdeposit,accountowner,depositorname,dates) VALUES('".$_SESSION['accountnumber']."', '".$Amount_to_deposit."', '".$_SESSION['username']."', '".$Depositor_name."','".$time."' )";
				...
				header('Location:\BankProject/EasyBank/HomePage.php? u have deposited ');
